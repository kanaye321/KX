...
export default function VMInventoryPage() {
  ...
  const handleEditVM = () => {
    ...
    toast({
      title: "VM Updated",
      description: `Virtual Machine ${vmToEdit.vmName} has been updated.`,
    });
  ...
  const handleImportVMs = () => {
    ...
    try {
      ...
      // Generate IDs for the new VMs
      const maxId = vms.length > 0 ? Math.max(...vms.map(vm => vm.id)) : 0;
      const vmsWithIds = newVMs.map((vm, index) => ({
        ...vm,
        id: maxId + index + 1
      }));
      
      // Update the VM list
      const updatedVMs = [...vms, ...vmsWithIds];
      queryClient.setQueryData(['/api/vms'], updatedVMs);
      
      toast({
        title: "Import Successful",
        description: `Imported ${vmsWithIds.length} virtual machines`,
      });
  ...
  const filteredVMs = vms.filter((vm) => {
    const matchesSearch = searchTerm === "" || 
      ...
      vm.vmId.toLowerCase().includes(searchTerm.toLowerCase()) ||
      vm.vmIp.includes(searchTerm);
    
    const matchesStatus = !statusFilter || statusFilter === "all" || 
      (vm.vmStatus && vm.vmStatus.toLowerCase() === (statusFilter || "").toLowerCase());
          ...
          <div className="overflow-x-auto">
            <Table>
              ...
              <TableBody>
                {filteredVMs.map((vm) => (
                  <TableRow key={vm.id}>
                    <TableCell>{getValidityBadge(vm.startDate, vm.endDate)}</TableCell>
                    <TableCell className="font-medium">{vm.vmId}</TableCell>
                    <TableCell>{vm.vmName}</TableCell>
                    <TableCell>{getStatusBadge(vm.vmStatus)}</TableCell>
                    <TableCell>{vm.hypervisor}</TableCell>
                    <TableCell>{vm.hostIp}</TableCell>
                    <TableCell>{vm.user}</TableCell>
                    <TableCell>{vm.department}</TableCell>
                    <TableCell>{vm.endDate}</TableCell>
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground mt-1">
                {importType === "csv" ? 
                  "The CSV file must include vmId, vmName, and hypervisor columns. Other fields are optional." :
                  "Save your Excel file as CSV first, then upload it here."
                }
              </p>
            </div>
